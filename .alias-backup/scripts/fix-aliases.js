...
function fixImportsInFile(filePath) {
  try {
    ...
    content = content.replace(
      ...
      (match, prefix, quote, importPath) => {
        ...
        if (targetFile) {
          ...
          stats.errors.push(
            `Could not find target for ${alias} in ${filePath}`
          );
          return match;
        }
      }
    ...
    content = content.replace(
      ...
      (match, prefix, quote, importPath, suffix) => {
        const alias = `@/${importPath}`;
        const targetFile = findTargetFile(alias, importPath, filePath);
...
function main() {
  ...
  if (!fs.existsSync(OUT_DIR)) {
    console.error(
      `❌ Error: ${OUT_DIR} does not exist. Run 'npm run build' first.`
    );
    process.exit(1);
  }
  ...
  files.forEach((file, index) => {
    if ((index + 1) % 50 === 0) {
      process.stdout.write(
        `   Processed ${index + 1}/${files.length} files...\r`
      );
    }
