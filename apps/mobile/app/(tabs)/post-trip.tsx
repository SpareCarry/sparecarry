...
import { LocationInput, FormTemplates, type TripTemplate } from '@sparecarry/ui';
...
export default function PostTripScreen() {
  ...
  const [submitStep, setSubmitStep] = useState<'idle' | 'validating' | 'submitting' | 'complete'>('idle');
          ...
          if (draft) {
            try {
              ...
              if (!fromLocation && !toLocation) {
                ...
                setProhibitedItemsConfirmed(draftData.prohibitedItemsConfirmed || false);
  ...
  }, [tripType, fromLocation, toLocation, fromLat, fromLon, toLat, toLon, spareKg, spareVolume, maxLength, maxWidth, maxHeight, prohibitedItemsConfirmed, canTakeOutboard, canTakeSpar, canTakeDinghy, canOversize, canTakeHazardous, departureDate, arrivalDate]);
  ...
  const handleSubmit = async () => {
    ...
    if (tripType === 'plane') {
      ...
      if (!spareKg || isNaN(spareKgValue) || spareKgValue < 0 || spareKgValue > 32) {
      ...
      if (!prohibitedItemsConfirmed) {
        validationErrors.prohibitedItems = 'You must confirm that you are not carrying prohibited items for plane transport';
      }
    } else {
      if (spareKg) {
    ...
    try {
      ...
      if (tripType === 'plane') {
        ...
        tripData.spare_volume_liters = spareVolume ? parseFloat(spareVolume) : null;
        ...
        {tripType === 'plane' ? (
          ...
          <>
            ...
            <View style={styles.field}>
              ...
              {[
                { key: 'outboard', label: 'Can take outboard motors', state: canTakeOutboard, setter: setCanTakeOutboard },
                { key: 'spar', label: 'Can take spars', state: canTakeSpar, setter: setCanTakeSpar },
                { key: 'dinghy', label: 'Can take dinghies', state: canTakeDinghy, setter: setCanTakeDinghy },
                { key: 'oversize', label: 'Can take oversize items', state: canOversize, setter: setCanOversize },
                { key: 'hazardous', label: 'Can take hazardous materials', state: canTakeHazardous, setter: setCanTakeHazardous },
