  ...
  try {
    ...
    if (supabase) {
      try {
        ...
        supabaseInitError = realtimeError instanceof Error ? realtimeError : new Error(String(realtimeError));
    ...
    console.error('');
    console.error('❌❌❌ CRITICAL: Failed to initialize Supabase client ❌❌❌');
...
function NavigationHandler() {
  ...
  useEffect(() => {
    ...
    if (!user && !inAuthGroup) {
      ...
      mobileLogger.warn('Not authenticated, redirecting to login', { route: currentRoute });
      router.replace('/auth/login');
    } else if (user && inAuthGroup) {
      // Redirect to home if authenticated and on auth screen
      mobileLogger.info('Authenticated, redirecting to tabs', { route: currentRoute });
      router.replace('/(tabs)');
    }
...
export default function RootLayout() {
  useEffect(() => {
    ...
    return () => {
      ...
      mobileLogger.info('Mobile app shutting down, cleaning up RealtimeManager');
      try {
