...
try {
  ...
  AutoMeasureCamera =
    AutoMeasureModule.AutoMeasureCamera || AutoMeasureModule.default;
  ...
  console.warn(
    "⚠️ [Auto-Measure] Failed to load AutoMeasureCamera module:",
    normalizedError
  );
  console.warn("   Auto-Measure feature will be disabled.");
  mobileLogger.warn("Auto-Measure module not available", {
...
export default function AutoMeasureScreen() {
  ...
  useEffect(() => {
    ...
    if (!AutoMeasureCamera) {
      setModuleError(
        "Auto-Measure feature is not available. The camera module could not be loaded."
      );
    }
  }, [params]);

  const handleMeasurementComplete = (result: any, photos?: any[]) => {
  ...
  return (
    <AutoMeasureCamera onMeasurementComplete={handleMeasurementComplete} />
  );
}
