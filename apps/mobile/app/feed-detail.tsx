...
async function fetchItemDetails(id: string, type: 'trip' | 'request'): Promise<FeedItem | null> {
...
async function createMatch(requesterId: string, travelerId: string, requestId?: string, tripId?: string): Promise<string | null> {
  const supabase = createClient();
  ...
  const { data: existingMatch } = await supabase
    .from('matches')
    .select('id')
    .or(`and(requester_id.eq.${requesterId},traveler_id.eq.${travelerId}),and(requester_id.eq.${travelerId},traveler_id.eq.${requesterId})`)
...
function openWhatsApp(phone: string, message: string) {
  ...
  Linking.openURL(whatsappUrl).catch((err) => {
    ...
    Linking.openURL(webUrl).catch((webErr) => {
      ...
      Alert.alert('Error', 'Could not open WhatsApp. Please install WhatsApp or use WhatsApp Web.');
    });
  });
}
...
export default function FeedDetailScreen() {
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      const matchId = await createMatch(requesterId, travelerId, requestId, tripId);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.title}>
          {isTrip ? `${item.from_location} → ${item.to_location}` : item.title || 'Delivery Request'}
        </Text>
            ...
            <>
              ...
              {item.length_cm && item.width_cm && item.height_cm && (
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText}>
                    Dimensions: {item.length_cm} × {item.width_cm} × {item.height_cm} cm
                  </Text>
                </View>
              )}
              {item.preferred_method && (
                <View style={styles.infoRow}>
                  <MaterialIcons
                    name={item.preferred_method === 'plane' ? 'flight' : item.preferred_method === 'boat' ? 'directions-boat' : 'swap-horiz'}
                    size={20}
                    color="#666"
                  />
                  <Text style={styles.infoText}>
        ...
        {!isInvolved && (
          <TouchableOpacity
            style={[styles.messageButton, (loading || loadingPhone) && styles.messageButtonDisabled]}
