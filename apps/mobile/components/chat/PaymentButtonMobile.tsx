...
export function PaymentButtonMobile({ match, matchId }: PaymentButtonMobileProps) {
  const { user } = useAuth();
  ...
  const { data: profileData } = useQuery({
    queryKey: ['user-profile-credits', user?.id],
    queryFn: async () => {
      if (!user) return null;
      const { data } = await supabase
        .from('profiles')
  ...
  const availableCredits = (profileData?.referral_credit_cents || 0) / 100;
  const maxCreditsUsable = Math.min(availableCredits, platformFee + reward);
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_APP_URL || 'http://localhost:3000'}/api/payments/create-intent`,
        {
          ...
          body: JSON.stringify({
            ...
            insurance: insurance ? { premium: 0, coverage_amount: itemCost } : null,
          }),
        }
      );
          ...
          <View style={styles.breakdown}>
            ...
            {itemCost > 0 && (
              <View style={styles.breakdownRow}>
                ...
                <Text style={styles.breakdownValue}>${itemCost.toFixed(2)}</Text>
              </View>
            )}
              ...
              <View style={styles.creditsSection}>
                ...
                >
                  ...
                  <Text style={styles.creditsLabel}>
                    Use ${maxCreditsUsable.toFixed(2)} referral credits
                  </Text>
                </TouchableOpacity>
              </View>
            )}
