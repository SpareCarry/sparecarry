...
const DEBUG_MODE_ENABLED = __DEV__ && process.env.EXPO_PUBLIC_DEBUG_MODE === 'true';
...
class MobileDebugMode {
  ...
  logModuleResolution(moduleName: string, resolvedPath: string, version?: string) {
    ...
    console.log(`[DEBUG] Module Resolution: ${moduleName} → ${resolvedPath}${version ? ` (v${version})` : ''}`);
  }
  ...
  getDebugInfo(): DebugInfo {
    try {
      ...
      return {
        ...
        reactNativeVersion: ReactNative.Version?.major + '.' + ReactNative.Version?.minor + '.' + ReactNative.Version?.patch || 'unknown',
  ...
  printDebugReport() {
    ...
    info.moduleResolutions.forEach((m) => {
      console.log(`  - ${m.moduleName}: ${m.resolvedPath}${m.version ? ` (v${m.version})` : ''}`);
    });
    console.log('');
...
if (DEBUG_MODE_ENABLED) {
  try {
    ...
    console.log('[DEBUG] React Native Version:', 
      ReactNative.Version?.major + '.' + ReactNative.Version?.minor + '.' + ReactNative.Version?.patch
    );
    
    mobileDebugMode.logModuleResolution('react', require.resolve('react'), React.version);
    mobileDebugMode.logModuleResolution('react-native', require.resolve('react-native'), 
      ReactNative.Version?.major + '.' + ReactNative.Version?.minor + '.' + ReactNative.Version?.patch
    );
  } catch (error) {
