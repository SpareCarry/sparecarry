...
const packageJsonBackupPath = path.join(projectRoot, 'package.json.pnpm-backup');
...
if (npmPackageJson.dependencies) {
  Object.keys(npmPackageJson.dependencies).forEach(key => {
    if (npmPackageJson.dependencies[key] === 'workspace:*') {
      const workspacePath = path.join(workspaceRoot, 'packages', key.replace('@sparecarry/', ''));
      if (fs.existsSync(workspacePath)) {
        npmPackageJson.dependencies[key] = `file:${workspacePath}`;
        console.log(`   Replaced ${key} with file: path`);
      } else {
        console.warn(`   ⚠️  Workspace package ${key} not found, keeping workspace:*`);
      }
    }
  });
}
