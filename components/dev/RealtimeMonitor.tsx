...
export function RealtimeMonitor() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-h-96 w-96 overflow-auto">
      <Card className="border-yellow-300 bg-yellow-50">
        ...
        <CardContent className="space-y-2 text-xs">
            ...
            {debugInfo.channels.map((ch) => (
              ...
              >
                ...
                <div className="text-slate-600">
                  Callbacks: {ch.callbacks} | Inactive:{" "}
                  {Math.round(ch.inactiveTime / 1000)}s
                </div>
                <div className="text-xs text-slate-500">
                  Table: {ch.config.table}
                  {ch.config.filter &&
                    ` | Filter: ${ch.config.filter.substring(0, 30)}...`}
                </div>
              </div>
            ))}
          </div>
          <Button
            onClick={() => {
              console.log(
                "RealtimeManager Debug Info:",
                RealtimeManager.getDebugInfo()
              );
            }}
            size="sm"
            variant="outline"
