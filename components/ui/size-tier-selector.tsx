...
export function SizeTierSelector({
  ...
  const tierInfo = selectedTier
    ? SIZE_TIERS.find((t) => t.id === selectedTier)
    : null;

  return (
    <div className={className} data-testid="size-tier-selector">
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="max-w-xs">
              <p className="text-sm">
                Select the size tier that best matches your package
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
      <Select value={selectedTier || undefined} onValueChange={onValueChange}>
      ...
      {tierInfo && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <p className="mt-1 cursor-help text-xs text-slate-500">
                Examples: {tierInfo.examples}
              </p>
            </TooltipTrigger>
            <TooltipContent className="max-w-xs">
              <div className="space-y-1">
                <p className="text-sm font-semibold">{tierInfo.label}</p>
