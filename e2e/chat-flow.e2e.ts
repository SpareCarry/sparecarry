...
describe("Chat Flow", () => {
  ...
  it("should navigate to messages screen", async () => {
    ...
    const messagesButton = element(by.id("messages-button")).or(
      by.text("Messages")
    );
  ...
  it("should display conversation list", async () => {
    ...
    await waitFor(
      element(by.id("conversation-list")).or(by.id("conversation-item"))
    )
      .toBeVisible()
      .withTimeout(10000);
  });
  ...
  it("should send a message", async () => {
    ...
    const messageInput = element(by.id("message-input")).or(
      by.label("Message")
    );
