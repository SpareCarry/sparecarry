...
describe("Listing Creation Flow", () => {
  ...
  it("should navigate to post request screen", async () => {
    ...
    const postRequestButton = element(by.id("post-request-button")).or(
      by.text("Post Request")
    );
    await waitFor(postRequestButton).toBeVisible().withTimeout(5000);
    await postRequestButton.tap();

    await waitFor(
      element(by.id("post-request-form")).or(by.text("Post Request"))
    )
      .toBeVisible()
      .withTimeout(3000);
  });
  ...
  it("should fill in request form", async () => {
    ...
    const fromInput = element(by.id("from-location-input")).or(
      by.label("From")
    );
  ...
  it("should submit request form", async () => {
    ...
    const submitButton = element(by.id("submit-request-button")).or(
      by.text("Submit")
    );
