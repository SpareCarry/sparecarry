...
class Logger {
  ...
  constructor() {
    ...
    this.samplingRate = process.env.NODE_ENV === 'production' 
  ...
  error(message: string, error?: Error | unknown, context?: LogContext): void {
    ...
    if (this.sentryEnabled && (this.samplingRate >= 1.0 || Math.random() < this.samplingRate)) {
   ...
   */
  async setUser(user: { id: string; email?: string; username?: string } | null): Promise<void> {
    if (!this.sentryEnabled || typeof window === 'undefined') return;
