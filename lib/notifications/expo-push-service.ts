 ...
 */
export async function registerForExpoPushNotifications(): Promise<
  string | null
> {
  if (typeof window === "undefined" || !isNativePlatform()) {
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      const errorListener = PushNotificationsPlugin.addListener(
        "registrationError",
        (error: unknown) => {
...
export async function sendExpoPushNotification(
  ...
  if (typeof window !== "undefined") {
    throw new Error(
      "sendExpoPushNotification must be called from server-side code"
    );
  }
