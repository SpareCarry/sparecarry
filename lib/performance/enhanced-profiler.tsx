...
class PerformanceAnalyzer {
  ...
  private renderHistory: Array<{ component: string; time: number; timestamp: number }> = [];
  ...
  recordNetworkRequest(
    ...
    existing.averageDuration = 
      (existing.averageDuration * (existing.callCount - 1) + duration) / existing.callCount;
  ...
  detectBottlenecks(): Bottleneck[] {
    ...
    for (const [name, metrics] of this.componentMetrics.entries()) {
      ...
      if (metrics.renderCount > 100 && metrics.averageRenderTime > 10) {
        bottlenecks.push({
          ...
          severity: metrics.averageRenderTime > this.CRITICAL_RENDER_THRESHOLD ? 'critical' : 'high',
      ...
      if (metrics.propsChanges > metrics.renderCount * 0.8 && metrics.renderCount > 20) {
        bottlenecks.push({
          component: name,
    ...
    for (const [endpoint, metrics] of this.networkMetrics.entries()) {
      ...
      if (metrics.cacheHitRate < 0.3 && metrics.callCount > 20 && endpoint.includes('autocomplete')) {
        bottlenecks.push({
          component: endpoint,
  ...
  getReport(): {
    ...
    const avgRenderTime = components.length > 0
      ? components.reduce((sum, m) => sum + m.averageRenderTime, 0) / components.length
      : 0;
    ...
    const avgNetworkTime = network.length > 0
      ? network.reduce((sum, m) => sum + m.averageDuration, 0) / network.length
      : 0;
...
export function usePerformanceTracking(
  ...
  useEffect(() => {
    ...
    const propsChanged = prevPropsRef.current && props
      ? JSON.stringify(prevPropsRef.current) !== JSON.stringify(props)
      : false;
    
    const stateChanged = prevStateRef.current !== undefined && state !== undefined
      ? JSON.stringify(prevStateRef.current) !== JSON.stringify(state)
      : false;

    performanceAnalyzer.recordRender(componentName, renderTime, propsChanged, stateChanged);
...
export function useNetworkTracking() {
  const trackRequest = useCallback(async <T,>(
    ...
    try {
      ...
      performanceAnalyzer.recordNetworkRequest(endpoint, duration, cached, false);
      return result;
    } catch (error) {
      const duration = performance.now() - start;
      performanceAnalyzer.recordNetworkRequest(endpoint, duration, cached, true);
      throw error;
    }
  }, []);
    ...
    <div style={{
      ...
      <div>
        ...
        <ul style={{ margin: '0.5rem 0', paddingLeft: '1.5rem' }}>
          ...
          <li>Avg Render Time: {report.summary.averageRenderTime.toFixed(2)}ms</li>
          <li>Slow Components: {report.summary.slowComponents}</li>
          <li>Network Calls: {report.summary.totalNetworkCalls}</li>
          <li>Avg Network Time: {report.summary.averageNetworkTime.toFixed(0)}ms</li>
        </ul>
      </div>
      {report.bottlenecks.length > 0 && (
        <div style={{ marginTop: '1rem' }}>
          <strong>Bottlenecks ({report.bottlenecks.length}):</strong>
          <ul style={{ margin: '0.5rem 0', paddingLeft: '1.5rem', fontSize: '11px' }}>
            {report.bottlenecks.slice(0, 5).map((b, i) => (
              <li key={i} style={{ color: b.severity === 'critical' ? '#ff6b6b' : b.severity === 'high' ? '#ffa500' : '#ffd700' }}>
