...
export async function hasCompletedDeliveries(userId: string): Promise<boolean> {
  ...
  try {
    ...
    return (
      ((data as { completed_deliveries_count?: number | null })
        .completed_deliveries_count || 0) > 0
    );
  } catch (error) {
 ...
 */
export async function getPromoCardToShow(
  userId?: string
): Promise<"early-supporter" | "first-delivery" | null> {
  const daysLeft = getDaysLeft();
