...
export default function (data) {
  ...
  if (createTrip) {
    ...
    const tripSuccess = check(tripRes, {
      "create trip status is 200 or 201": (r) =>
    ...
    const requestSuccess = check(requestRes, {
      "create request status is 200 or 201": (r) =>
        r.status === 200 || r.status === 201,
    ...
    if (
      requestSuccess &&
      (requestRes.status === 200 || requestRes.status === 201)
    ) {
      try {
  ...
  if (entityId && entityType) {
    ...
    const matchesRes = http.get(
      `${apiUrl}/matches?${entityType}_id=${entityId}`,
      {
        headers: headers,
