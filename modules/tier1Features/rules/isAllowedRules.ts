...
export function isAllowedToCarry(details: ItemDetails): AllowedResult {
  ...
  if (details.hasBatteries) {
    ...
    warnings.push(
      "⚠️ Contains batteries - ensure proper packaging to prevent short circuits"
    );
    warnings.push(
      "Lithium batteries may have airline restrictions - check with carrier"
    );
  }
  ...
  if (details.hasLiquids) {
    if (details.liquidVolume && details.liquidVolume > 100) {
      restrictions.push(
        "Liquid volume exceeds 100ml - may not be allowed in carry-on"
      );
      warnings.push("Large liquids may need to be in checked luggage");
    } else {
      warnings.push(
        "Contains liquids - ensure sealed and leak-proof packaging"
      );
    }
  }
  ...
  if (details.isFood || details.category?.toLowerCase().includes("food")) {
    allowed = true;
    warnings.push(
      "🌍 Food items may be restricted by customs - check destination country regulations"
    );
  ...
  if (
    details.isMedicine ||
    details.category?.toLowerCase().includes("medicine")
  ) {
    allowed = true;
    restrictions.push(
      "💊 Prescription medicines require original packaging and documentation"
    );
    warnings.push(
      "Some medicines may be restricted or require permits in certain countries"
    );
  ...
  for (const keyword of dangerousKeywords) {
    if (text.includes(keyword)) {
      allowed = false;
      restrictions.push(
        `❌ Item contains dangerous goods (${keyword}) - NOT ALLOWED`
      );
      break;
    }
  }
