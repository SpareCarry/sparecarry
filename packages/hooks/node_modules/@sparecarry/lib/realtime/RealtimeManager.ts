 ...
 * - Connection limits: Hard limit to prevent runaway connections (MAX 5)
...
class RealtimeManagerClass {
  ...
  private readonly MAX_CHANNELS = 5; // Reduced from 10 to 5 for quota compliance
   ...
   * Must be called before using listen()
   */
  ...
  /**
   * Get the Supabase client (throws if not set)
   */
  private getSupabaseClient(): SupabaseClient {
    if (!this.supabase) {
      throw new Error('RealtimeManager: Supabase client not set. Call setSupabaseClient() first.');
    }
    return this.supabase;
  }

  constructor() {
    // Start cleanup interval (works on both web and mobile)
    this.cleanupInterval = setInterval(() => {
      this.cleanupInactiveChannels();
    }, 60000); // Check every minute

    // Cleanup on page unload (web only)
  ...
  listen(
    ...
    const channel = supabase.channel(customChannelName || `${fullConfig.table}-${Date.now()}`) as RealtimeChannel;
...
// Expose in dev tools for debugging (web only)
