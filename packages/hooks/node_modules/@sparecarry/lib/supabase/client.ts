...
function createExpoStorage() {
  if (!SecureStore) {
    throw new Error(
      "expo-secure-store is not available. This function should only be called on mobile."
    );
  }
  ...
  return {
    ...
    removeItem: async (key: string): Promise<void> => {
      try {
        ...
        console.error(
          "[Supabase] Error removing item from SecureStore:",
          error
        );
      }
    },
...
function createMobileClient(): SupabaseClient {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    ...
    console.warn(
      "⚠️ Creating placeholder Supabase client. API calls will fail."
    );
  ...
  try {
    ...
    console.warn(
      "⚠️ Falling back to in-memory storage (sessions won't persist)"
    );
    storage = {
      getItem: async () => null,
      setItem: async () => {},
  ...
  try {
        ...
        fetch: async (url, options = {}) => {
          ...
          try {
            ...
            if (error.name === "AbortError") {
              ...
              console.error(
                "   This might be a connectivity issue. Check your network connection."
              );
            } else {
    ...
    client.auth.getSession().catch((err) => {
      if (
        ...
        console.warn(
          "⚠️ [Supabase] Connection test failed - network error (this is OK if offline):",
          err.message
        );
      } else {
        console.warn(
          "⚠️ [Supabase] Connection test failed (this is OK if offline):",
          err.message
        );
      }
    });
...
export function createClient(): TypedSupabaseClient {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    ...
    return isMobile
      ? mobileClientInstance || browserClientInstance!
