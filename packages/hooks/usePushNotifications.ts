...
/// <reference path="./expo-constants.d.ts" />
...
async function loadNotificationsModule() {
  ...
  try {
    ...
    console.warn(
      "[usePushNotifications] Failed to load expo-notifications:",
      error
    );
  ...
  useEffect(() => {
    ...
    loadNotificationsModule()
      .then(({ Notifications: Notifs, Device: Dev }) => {
        ...
        if (!Notifs || !Dev) {
          console.warn(
            "[usePushNotifications] expo-notifications not available (Expo Go limitation)"
          );
          setLoading(false);
          return;
        }
        ...
        responseListener.current =
          Notifs.addNotificationResponseReceivedListener(
...
async function registerForPushNotificationsAsync(
  ...
  if (Device.isDevice) {
    ...
    try {
      ...
      // Fallback to Expo config extra.eas.projectId if env vars are not set
      if (!projectId || projectId === "your-project-id") {
        try {
          const { default: Constants } = await import("expo-constants");
          projectId =
            (Constants?.expoConfig as any)?.extra?.eas?.projectId ?? projectId;
        } catch (constantsError) {
          // If Constants cannot be loaded, we'll fall back to env-only behavior
        }
      }

      if (!projectId || projectId === "your-project-id") {
        console.warn(
          "[usePushNotifications] EAS_PROJECT_ID not set. Push notifications will not work."
        );
        console.warn(
          "   Set EXPO_PUBLIC_EAS_PROJECT_ID in apps/mobile/.env.local or ensure app.config.ts extra.eas.projectId is correct."
        );
        return null;
      }
      ...
      if (
        e?.message?.includes("FirebaseApp") ||
        e?.message?.includes("Firebase")
      ) {
        console.warn(
          "[usePushNotifications] Firebase not configured. Push notifications disabled."
        );
        console.warn(
          "   To enable: Follow https://docs.expo.dev/push-notifications/fcm-credentials/"
        );
        return null;
      }
