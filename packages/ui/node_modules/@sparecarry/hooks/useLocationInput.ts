...
export function useLocationInput() {
   ...
   */
  const handleGetCurrentLocation = useCallback(async (): Promise<LocationInputValue | null> => {
    ...
    try {
      ...
      try {
        ...
        const address = geocoded?.formatted || `${location.latitude.toFixed(4)}, ${location.longitude.toFixed(4)}`;
        
        // Save to recent locations
        await saveRecentLocation(address, location.latitude, location.longitude);
        ...
        await saveRecentLocation(coordAddress, location.latitude, location.longitude);
      ...
      Alert.alert('Location Error', error.message || 'Failed to get location. Please enter manually.');
      return null;
    } finally {
   ...
   */
  const saveRecentLocation = useCallback(async (address: string, lat: number, lon: number) => {
    try {
   ...
   */
  const getRecentLocations = useCallback(async (): Promise<RecentLocation[]> => {
    try {
