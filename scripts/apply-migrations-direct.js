...
async function executeSQLViaAPI(supabaseUrl, serviceKey, sql) {
    return new Promise((resolve, reject) => {
        ...
        resolve({ success: false, message: 'Direct SQL execution not available via REST API' });
    });
}
...
async function main() {
    ...
    if (existingTables.length > 0) {
        log(`✅ Existing tables (${existingTables.length}): ${existingTables.join(', ')}`, 'green');
    }
    
    if (missingTables.length > 0) {
        log(`❌ Missing tables (${missingTables.length}): ${missingTables.join(', ')}`, 'red');
    }
    
    if (missingTables.length === 0) {
        log('\n✅ All tables exist! Database appears to be set up.', 'green');
        log('   If you want to re-seed data, run: pnpm db:seed:staging --reset\n', 'yellow');
        return;
    }
    
    log('\n⚠️  Direct SQL execution via Supabase JS client is not supported', 'yellow');
    ...
    log('   1. Open: https://supabase.com/dashboard/project/gujyzwqcwecbeznlablx', 'cyan');
    ...
    log('✅ Run migrations via Dashboard, then verify with: pnpm db:setup\n', 'green');
}
