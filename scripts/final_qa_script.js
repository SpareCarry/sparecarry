...
(async () => {
  ...
  // Test 2: Build Check
  runTest("Build Check", "node scripts/preflight-beta.js", false);
  ...
  const packageManager = fs.existsSync(
    ...
    ? "pnpm"
    : "npm";
  runTest("Unit Tests (Vitest)", `${packageManager} test`, false);

  // Test 4: Type Check
  runTest("TypeScript Type Check", `${packageManager} typecheck`, false);

  // Test 5: Lint Check
  runTest("Lint Check", `${packageManager} lint`, false);

  // Test 6: Build (if not in CI or if explicitly requested)
  if (!CI_MODE || process.argv.includes("--build")) {
    runTest("Staging Build", `${packageManager} build:staging`, false);
  }
