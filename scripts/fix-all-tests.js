...
function extractSummary(output) {
  ...
  const filesMatch = output.match(
    /Test Files.*?(\d+) failed.*?(\d+) passed.*?\((\d+)\)/
  );
  if (filesMatch) {
...
function getErrorSignature(output) {
  ...
  // Extract error types
  if (output.includes("createInlineMock is not defined"))
    errors.push("mock-definition");
  if (output.includes("Vitest cannot be imported"))
    errors.push("vitest-import");
  ...
  if (output.includes("Found multiple elements"))
    errors.push("multiple-elements");
...
function applyFixes(output) {
  ...
  if (
    output.includes("Found multiple elements") &&
    ...
    if (
      content.includes("screen.getByText(/required/i)") &&
  ...
  if (
    ...
    output.includes("Vitest cannot be imported")
  ) {
    ...
    if (
      ...
      content.includes("createInlineMock")
    ) {
  ...
  if (
    ...
    let autoMatchContent = fs.readFileSync(
    ...
    let createIntentContent = fs.readFileSync(
...
while (iteration < MAX_ITERATIONS) {
  ...
  console.log(`\n📊 Summary:`);
  console.log(
    `   Test Files: ${summary.failed} failed | ${summary.passed} passed | ${summary.totalFiles} total`
  );
  ...
  if (errorSig === lastErrorHash && lastErrorHash !== "" && iteration > 1) {
    ...
    console.log(
      "   Stopping automated fixes. Manual intervention may be needed.\n"
    );
