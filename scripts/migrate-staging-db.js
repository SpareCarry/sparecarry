...
if (!SUPABASE_URL) {
  log(
    "❌ Error: STAGING_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_URL not set",
    "red"
  );
...
if (!SERVICE_KEY) {
  log(
    "❌ Error: STAGING_SUPABASE_SERVICE_ROLE_KEY or SUPABASE_SERVICE_ROLE_KEY not set",
    "red"
  );
  log("   Set in .env.staging or .env.local", "yellow");
  process.exit(1);
  ...
  for (const statement of statements) {
    ...
    try {
      ...
      if (error) {
        ...
        log(
          `⚠️  RPC method not available. Please run SQL manually in Supabase Dashboard.`,
          "yellow"
        );
        log(`   File: ${sqlFile}\n`, "cyan");
        return;
      ...
      log(
        `⚠️  Direct SQL execution not available. Please run SQL manually in Supabase Dashboard.`,
        "yellow"
      );
...
(async () => {
  ...
  log(
    "⚠️  Note: Some SQL operations require direct database access.",
    "yellow"
  );
  log(
    "   If migrations fail, run SQL files manually in Supabase SQL Editor.\n",
    "yellow"
  );
})();
