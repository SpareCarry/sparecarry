...
function fixImportsInFile(filePath) {
  try {
    ...
    // Create backup
    const backupPath = path.join(BACKUP_DIR, path.relative(PROJECT_ROOT, filePath));
    ...
    newContent = newContent.replace(es6ImportPattern, (match, prefix, quote, importPath) => {
...
function findSourceFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      if (!['node_modules', '.next', 'out', '.alias-backup'].includes(file) && !filePath.includes('app/api')) {
...
console.log('💡 Run "node scripts/pre-build-fix-aliases.js restore" to restore original files');
