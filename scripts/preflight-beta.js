...
try {
  ...
  if (gitStatus.trim() === '') {
    ...
    console.warn('⚠️  Git working directory clean: Uncommitted changes detected (non-blocking)');
  }
} catch {
  checks.warnings.push('Git working directory clean');
  console.warn('⚠️  Git working directory clean: Not a git repository or git not available (non-blocking)');
}
...
try {
  ...
  if (fs.existsSync(validateEnvScript)) {
    ...
    check('Environment variables validated', false, 'validate-env.js script not found');
  }
} catch (error) {
  check('Environment variables validated', false, 'Environment validation failed - check errors above');
}
