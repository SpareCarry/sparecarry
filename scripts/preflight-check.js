...
check(
  "Capacitor installed",
  deps["@capacitor/core"],
  "Install: pnpm add @capacitor/core"
);
...
check(
  ...
  fs.existsSync("tailwind.config.js") || fs.existsSync("tailwind.config.ts")
);
...
check(
  ...
  fs.existsSync(".env.local.example"),
...
check(
  "lib/security/rate-limit.ts exists",
  fs.existsSync("lib/security/rate-limit.ts")
);
check(
  "lib/security/auth-guards.ts exists",
  fs.existsSync("lib/security/auth-guards.ts")
);
check(
  "lib/security/validation.ts exists",
  fs.existsSync("lib/security/validation.ts")
);
check(
  "lib/api/error-handler.ts exists",
  fs.existsSync("lib/api/error-handler.ts")
);
...
check(
  "android/fastlane/Fastfile exists",
  fs.existsSync("android/fastlane/Fastfile")
);

// 10. Check backup scripts
check(
  "scripts/backup/backup_db.sh exists",
  fs.existsSync("scripts/backup/backup_db.sh")
);
check(
  "scripts/backup/restore_db.sh exists",
  fs.existsSync("scripts/backup/restore_db.sh")
);

// 11. Check load testing
check(
  "load-tests/scripts/browse.js exists",
  fs.existsSync("load-tests/scripts/browse.js")
);

// 12. Check feature flags
check(
  "lib/flags/unleashClient.ts exists",
  fs.existsSync("lib/flags/unleashClient.ts")
);
check(
  "app/providers/FeatureFlagProvider.tsx exists",
  fs.existsSync("app/providers/FeatureFlagProvider.tsx")
);

// 13. Check performance instrumentation
check(
  "lib/performance/web-profiler.ts exists",
  fs.existsSync("lib/performance/web-profiler.ts")
);
check(
  "lib/performance/db-profiler.ts exists",
  fs.existsSync("lib/performance/db-profiler.ts")
);
...
check(
  ".github/workflows/mobile-deploy.yml exists",
  fs.existsSync(".github/workflows/mobile-deploy.yml")
);
check(
  ".github/workflows/nightly-backup.yml exists",
  fs.existsSync(".github/workflows/nightly-backup.yml")
);
check(
  ".github/workflows/loadtest.yml exists",
  fs.existsSync(".github/workflows/loadtest.yml")
);

// 15. Check documentation
check(
  "ERROR_LOGGING_SYSTEM.md exists",
  fs.existsSync("ERROR_LOGGING_SYSTEM.md")
);
check(
  "MOBILE_DEPLOYMENT_AUTOMATION.md exists",
  fs.existsSync("MOBILE_DEPLOYMENT_AUTOMATION.md")
);
check(
  "BACKUP_RECOVERY_PLAYBOOK.md exists",
  fs.existsSync("BACKUP_RECOVERY_PLAYBOOK.md")
);
check(
  "FEATURE_FLAGS_README.md exists",
  fs.existsSync("FEATURE_FLAGS_README.md")
);
