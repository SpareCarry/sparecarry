...
for (let i = 0; i < testSuites.length; i++) {
  ...
  try {
    ...
    const hasPassed = !output.includes('FAILED') && 
                      !output.includes('failed') && 
                      (output.includes('PASSED') || output.includes('passed') || output.includes('✅'));
...
// Save master report
const reportSave = saveReportToFile(masterReport, 'test-results-ALL-TESTS-detailed.txt');
