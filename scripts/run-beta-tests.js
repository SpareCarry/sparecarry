...
try {
  const output = execSync('npm run typecheck', { encoding: 'utf-8', stdio: 'pipe' });
...
try {
  const output = execSync('npm run test:e2e', { encoding: 'utf-8', stdio: 'pipe', maxBuffer: 10 * 1024 * 1024 });
...
try {
  const output = execSync('npm run coverage', { encoding: 'utf-8', stdio: 'pipe' });
...
const reportResults = [
  { feature: 'TypeScript Type Check', passed: results.typecheck.status === 'passed', output: results.typecheck.output, error: results.typecheck.status === 'failed' ? 'Type check failed' : null },
  { feature: 'Unit Tests', passed: results.unitTests.status === 'passed', output: results.unitTests.output, error: results.unitTests.status === 'failed' ? 'Unit tests failed' : null },
  { feature: 'E2E Tests', passed: results.e2eTests.status === 'passed', output: results.e2eTests.output, error: results.e2eTests.status === 'failed' ? 'E2E tests failed' : null },
  { feature: 'Test Coverage', passed: results.coverage.status === 'passed', output: results.coverage.output, error: results.coverage.status === 'failed' ? 'Coverage generation failed' : null },
];
...
// Save results
const resultsFile = path.join(TEST_RESULTS_DIR, `beta-test-results-${Date.now()}.json`);
fs.writeFileSync(resultsFile, JSON.stringify(results, null, 2));

// Save detailed report
const reportSave = saveReportToFile(report, 'test-results-beta-detailed.txt', TEST_RESULTS_DIR);
...
const summary = {
  ...
  allPassed: results.typecheck.status === 'passed' && 
