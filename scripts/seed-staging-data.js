...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.error(`${colors.red}❌ Error: Supabase URL and Service Role Key required${colors.reset}`);
...
async function createTestUsers() {
  ...
  const testUsers = [
    { email: 'test-traveler1@sparecarry.test', password: 'Test123!@#', role: 'traveler', name: 'Test Traveler 1' },
    { email: 'test-traveler2@sparecarry.test', password: 'Test123!@#', role: 'traveler', name: 'Test Traveler 2' },
    { email: 'test-requester1@sparecarry.test', password: 'Test123!@#', role: 'requester', name: 'Test Requester 1' },
    { email: 'test-requester2@sparecarry.test', password: 'Test123!@#', role: 'requester', name: 'Test Requester 2' },
    { email: 'test-sailor1@sparecarry.test', password: 'Test123!@#', role: 'sailor', name: 'Test Sailor 1' },
  ];

  const usersEmpty = await checkTableEmpty('users');
  
  if (!usersEmpty) {
    console.log(`${colors.yellow}⚠️  Users table not empty, skipping user creation${colors.reset}`);
    // Fetch existing users for IDs
    const { data: existingUsers } = await supabase.from('users').select('id, email').limit(5);
    if (existingUsers) {
  ...
  for (const userData of testUsers) {
    try {
      ...
      if (authError) {
        console.error(`${colors.red}❌ Failed to create auth user ${userData.email}: ${authError.message}${colors.reset}`);
        continue;
      }
      ...
      if (userError) {
        console.error(`${colors.red}❌ Failed to create user record: ${userError.message}${colors.reset}`);
        continue;
      }
      ...
      if (profileError) {
        console.error(`${colors.yellow}⚠️  Profile creation warning: ${profileError.message}${colors.reset}`);
      } else {
        insertedIds.profiles.push(profile.id);
      }

      insertedIds.users.push(userId);
      console.log(`${colors.green}✅ Created user: ${userData.email} (${userId})${colors.reset}`);
    } catch (error) {
...
async function createTestTrips() {
  ...
  if (insertedIds.users.length < 2) {
    console.log(`${colors.yellow}⚠️  Not enough users, skipping trips${colors.reset}\n`);
    return;
  }
  ...
  if (!tripsEmpty) {
    console.log(`${colors.yellow}⚠️  Trips table not empty, skipping trip creation${colors.reset}\n`);
    return;
  }
  ...
  const testTrips = [
    {
      ...
      eta_window_start: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 6 * 60 * 60 * 1000).toISOString(),
      eta_window_end: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 8 * 60 * 60 * 1000).toISOString(),
      spare_kg: 23.0,
      spare_volume_liters: 50.0,
      max_dimensions: JSON.stringify({ length_cm: 150, width_cm: 50, height_cm: 30 }),
      can_oversize: false,
      status: 'active',
    },
    {
      ...
      departure_date: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 14 days from now
      eta_window_start: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000 + 12 * 60 * 60 * 1000).toISOString(),
      eta_window_end: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000 + 18 * 60 * 60 * 1000).toISOString(),
      spare_kg: 45.0,
      spare_volume_liters: 100.0,
      max_dimensions: JSON.stringify({ length_cm: 200, width_cm: 80, height_cm: 50 }),
      can_oversize: true,
      status: 'active',
    },
    {
      ...
      departure_date: new Date(Date.now() + 21 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 21 days from now
      eta_window_start: new Date(Date.now() + 21 * 24 * 60 * 60 * 1000 + 4 * 60 * 60 * 1000).toISOString(),
      eta_window_end: new Date(Date.now() + 21 * 24 * 60 * 60 * 1000 + 6 * 60 * 60 * 1000).toISOString(),
      spare_kg: 15.0,
      spare_volume_liters: 30.0,
      max_dimensions: JSON.stringify({ length_cm: 100, width_cm: 40, height_cm: 25 }),
      can_oversize: false,
      status: 'active',
    },
  ];
  ...
  if (error) {
    console.error(`${colors.red}❌ Failed to create trips: ${error.message}${colors.reset}`);
    return;
  }

  insertedIds.trips = trips.map(t => t.id);
  console.log(`${colors.green}✅ Created ${trips.length} test trips${colors.reset}\n`);
}
...
async function createTestRequests() {
  ...
  if (insertedIds.users.length < 3) {
    console.log(`${colors.yellow}⚠️  Not enough users, skipping requests${colors.reset}\n`);
    return;
  }
  ...
  if (!requestsEmpty) {
    console.log(`${colors.yellow}⚠️  Requests table not empty, skipping request creation${colors.reset}\n`);
    return;
  }
  ...
  const testRequests = [
    {
      ...
      description: 'Need a 12V deep cycle marine battery for my boat. Group 27 size preferred.',
      from_location: 'West Marine, San Francisco, CA',
      to_location: 'Grenada Marine, St. George\'s, Grenada',
      deadline_earliest: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      deadline_latest: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      max_reward: 150.00,
      weight_kg: 25.0,
      dimensions_cm: JSON.stringify({ length_cm: 30, width_cm: 17, height_cm: 20 }),
      value_usd: 250.00,
    ...
    },
    {
      ...
      description: 'Looking for a Lewmar winch handle, size 8. Any condition as long as it works.',
      from_location: 'SVB, Miami, FL',
      to_location: 'Rodney Bay Marina, Saint Lucia',
      deadline_earliest: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      deadline_latest: new Date(Date.now() + 18 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      max_reward: 75.00,
      weight_kg: 2.0,
      dimensions_cm: JSON.stringify({ length_cm: 40, width_cm: 5, height_cm: 5 }),
      value_usd: 120.00,
    ...
    },
    {
      ...
      description: 'URGENT: Need a prop shaft seal for Yanmar 3YM30. Part number: 128250-42590',
      from_location: 'Amazon, Los Angeles, CA',
      to_location: 'Bridgetown, Barbados',
      deadline_earliest: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      deadline_latest: new Date(Date.now() + 22 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      max_reward: 200.00,
      weight_kg: 0.5,
      dimensions_cm: JSON.stringify({ length_cm: 10, width_cm: 10, height_cm: 5 }),
      value_usd: 80.00,
    ...
    },
    {
      ...
      description: 'Looking for a quality foul weather jacket, size Large. Mustang or Helly Hansen preferred.',
      from_location: 'West Marine, San Diego, CA',
      to_location: 'Grenada Marine, St. George\'s, Grenada',
      deadline_earliest: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      deadline_latest: new Date(Date.now() + 12 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      max_reward: 100.00,
      weight_kg: 1.5,
      dimensions_cm: JSON.stringify({ length_cm: 40, width_cm: 30, height_cm: 10 }),
      value_usd: 300.00,
      preferred_method: 'any',
      emergency: false,
      purchase_retailer: 'west_marine',
    ...
    },
    {
      ...
      description: 'Need a handheld VHF radio. Standard Horizon or ICOM preferred.',
      from_location: 'SVB, Fort Lauderdale, FL',
      to_location: 'Rodney Bay Marina, Saint Lucia',
      deadline_earliest: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      deadline_latest: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      max_reward: 125.00,
      weight_kg: 0.8,
      dimensions_cm: JSON.stringify({ length_cm: 20, width_cm: 10, height_cm: 8 }),
      value_usd: 200.00,
      preferred_method: 'any',
      emergency: false,
      purchase_retailer: 'svb',
  ...
  if (error) {
    console.error(`${colors.red}❌ Failed to create requests: ${error.message}${colors.reset}`);
    return;
  }

  insertedIds.requests = requests.map(r => r.id);
  console.log(`${colors.green}✅ Created ${requests.length} test requests${colors.reset}\n`);
}
...
async function createTestMatches() {
  ...
  if (insertedIds.trips.length < 2 || insertedIds.requests.length < 3) {
    console.log(`${colors.yellow}⚠️  Not enough trips or requests, skipping matches${colors.reset}\n`);
    return;
  }
  ...
  if (!matchesEmpty) {
    console.log(`${colors.yellow}⚠️  Matches table not empty, skipping match creation${colors.reset}\n`);
    return;
  }
  ...
  if (error) {
    console.error(`${colors.red}❌ Failed to create matches: ${error.message}${colors.reset}`);
    return;
  }
  ...
  console.log(`${colors.green}✅ Created ${matches.length} test matches${colors.reset}\n`);
}
...
async function createTestMessages() {
  ...
  if (insertedIds.conversations.length === 0 || insertedIds.users.length < 2) {
    console.log(`${colors.yellow}⚠️  Not enough conversations or users, skipping messages${colors.reset}\n`);
    return;
  }
  ...
  if (!messagesEmpty) {
    console.log(`${colors.yellow}⚠️  Messages table not empty, skipping message creation${colors.reset}\n`);
    return;
  }
  ...
  if (!matchDetails || matchDetails.length === 0) {
    console.log(`${colors.yellow}⚠️  No match details found, skipping messages${colors.reset}\n`);
    return;
  }
      ...
      testMessages.push(
        {
          ...
          content: 'Hi! I can help with this delivery. When would be a good time to meet?',
        },
        {
          conversation_id: conversationId,
  ...
  if (testMessages.length > 0) {
    ...
    if (error) {
      console.error(`${colors.red}❌ Failed to create messages: ${error.message}${colors.reset}`);
    } else {
      insertedIds.messages = messages.map(m => m.id);
      console.log(`${colors.green}✅ Created ${messages.length} test messages${colors.reset}\n`);
    }
  }
}
...
async function createTestDelivery() {
  console.log(`${colors.blue}Creating test delivery with dispute...${colors.reset}`);

  if (insertedIds.matches.length < 3) {
    console.log(`${colors.yellow}⚠️  Not enough matches, skipping delivery${colors.reset}\n`);
    return;
  }
  ...
  if (!deliveriesEmpty) {
    console.log(`${colors.yellow}⚠️  Deliveries table not empty, skipping delivery creation${colors.reset}\n`);
    return;
  }
  ...
  const { data: delivery, error } = await supabase
    ...
    .insert({
      ...
      proof_photos: ['https://example.com/proof1.jpg', 'https://example.com/proof2.jpg'],
      gps_lat_long: JSON.stringify({ lat: 12.0522, lng: -61.7556 }),
      delivered_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), // 2 days ago
      dispute_opened_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), // 1 day ago
    })
    .select()
    .single();

  if (error) {
    console.error(`${colors.red}❌ Failed to create delivery: ${error.message}${colors.reset}`);
    return;
  }
  ...
  console.log(`${colors.green}✅ Created test delivery with dispute${colors.reset}\n`);
}

// Reset mode: Delete all test data
async function resetData() {
  console.log(`${colors.yellow}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`);
  console.log(`${colors.yellow}⚠️  RESET MODE: This will delete ALL data!${colors.reset}`);
  console.log(`${colors.yellow}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}\n`);
  ...
  return new Promise((resolve, reject) => {
    rl.question(`${colors.red}Are you sure you want to delete ALL data? (type 'yes' to confirm): ${colors.reset}`, async (answer) => {
      rl.close();
      if (answer !== 'yes') {
      ...
      try {
        ...
        for (const table of tables) {
          try {
              ...
              if (users && users.length > 0) {
                ...
                for (const user of users) {
                  try {
                    ...
                    console.error(`${colors.yellow}⚠️  Could not delete auth user ${user.id}: ${error.message}${colors.reset}`);
                  }
                }
              }
            }
            ...
            if (error && error.code !== 'PGRST116') {
              console.error(`${colors.red}❌ Failed to delete from ${table}: ${error.message}${colors.reset}`);
            } else {
              console.log(`${colors.green}✅ Deleted all records from ${table}${colors.reset}`);
            }
          } catch (error) {
...
async function main() {
  try {
    ...
    if (shouldReset) {
      await resetData();
      console.log(`${colors.blue}Re-seeding fresh test data...${colors.reset}\n`);
    }
    ...
    console.error(`${colors.red}❌ Seed failed: ${error.message}${colors.reset}`);
    console.error(error);
    process.exit(1);
  }
}
