...
try {
  ...
  const locationServicePath = path.join(
    __dirname,
  ...
  if (fs.existsSync(locationServicePath)) {
    ...
    const hasAutocomplete = content.includes(
      "export async function autocomplete"
    );
    const hasReverseGeocode = content.includes(
      "export async function reverseGeocode"
    );
    const hasForwardGeocode = content.includes(
...
try {
  const shippingServicePath = path.join(
    __dirname,
  ...
  if (fs.existsSync(shippingServicePath)) {
    ...
    // Check for key exports
    const hasCalculateEstimate = content.includes(
      "export function calculateShippingEstimate"
    );
    const hasCourierRates = content.includes("export function getCourierRates");
    const hasCustomsRates = content.includes(
      "export function calculateCustomsCost"
    );
    const hasSpareCarryPricing = content.includes(
      "calculateSpareCarryPlanePrice"
    );
    const hasLazyLoading =
      content.includes("loadCourierRates") ||
      content.includes("loadCustomsRates");

    if (
      hasCalculateEstimate &&
      hasCourierRates &&
      hasCustomsRates &&
      hasSpareCarryPricing
    ) {
...
try {
  ...
  componentsToCheck.forEach((componentPath) => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      const usesNewService =
        content.includes("lib/services/location") ||
  ...
  console.log(
    `   ✅ ${updatedCount}/${componentsToCheck.length} components updated`
  );
} catch (error) {
...
try {
  const profilerPath = path.join(
    __dirname,
    "../lib/performance/enhanced-profiler.tsx"
  );
  const fs = require("fs");
...
try {
  const providerPath = path.join(
    __dirname,
    "../app/providers/performance-provider.tsx"
  );
  const fs = require("fs");
