...
if (!fs.existsSync(envFile)) {
  console.error(`${colors.red}❌ Environment file not found: ${envFile}${colors.reset}`);
...
validate(
  ...
  validateLength(envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY, 100, 200, 'NEXT_PUBLIC_SUPABASE_ANON_KEY'),
...
validate(
  ...
  !envVars.SUPABASE_SERVICE_ROLE_KEY || validateLength(envVars.SUPABASE_SERVICE_ROLE_KEY, 100, 200, 'SUPABASE_SERVICE_ROLE_KEY'),
...
validate(
  ...
  validateKey(envVars.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY, stripeKeyPrefix, 'NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY'),
...
validate(
  'STRIPE_SECRET_KEY',
  !envVars.STRIPE_SECRET_KEY || validateKey(envVars.STRIPE_SECRET_KEY, stripeSecretPrefix, 'STRIPE_SECRET_KEY'),
...
validate(
  ...
  !envVars.STRIPE_WEBHOOK_SECRET || validateKey(envVars.STRIPE_WEBHOOK_SECRET, 'whsec_', 'STRIPE_WEBHOOK_SECRET'),
...
validate(
  ...
  !envVars.NEXT_PUBLIC_SENTRY_DSN || validateUrl(envVars.NEXT_PUBLIC_SENTRY_DSN, 'NEXT_PUBLIC_SENTRY_DSN'),
...
if (envVars.NEXT_PUBLIC_SENTRY_DSN) {
  validate(
    ...
    envVars.NEXT_PUBLIC_SENTRY_DSN.includes('@') && envVars.NEXT_PUBLIC_SENTRY_DSN.includes('.ingest.sentry.io'),
...
validate(
  ...
  !envVars.NEXT_PUBLIC_UNLEASH_URL || validateUrl(envVars.NEXT_PUBLIC_UNLEASH_URL, 'NEXT_PUBLIC_UNLEASH_URL'),
...
validate(
  ...
  !envVars.NEXT_PUBLIC_UNLEASH_CLIENT_KEY || validateLength(envVars.NEXT_PUBLIC_UNLEASH_CLIENT_KEY, 20, 200, 'NEXT_PUBLIC_UNLEASH_CLIENT_KEY'),
...
validate(
  'EXPO_ACCESS_TOKEN',
  !envVars.EXPO_ACCESS_TOKEN || validateLength(envVars.EXPO_ACCESS_TOKEN, 20, 200, 'EXPO_ACCESS_TOKEN'),
...
validate(
  'FCM_SERVER_KEY',
  !envVars.FCM_SERVER_KEY || validateLength(envVars.FCM_SERVER_KEY, 100, 200, 'FCM_SERVER_KEY'),
...
validate(
  'GOOGLE_CLIENT_ID',
  !envVars.GOOGLE_CLIENT_ID || validateLength(envVars.GOOGLE_CLIENT_ID, 20, 200, 'GOOGLE_CLIENT_ID'),
...
validate(
  'APPLE_CLIENT_ID',
  !envVars.APPLE_CLIENT_ID || validateLength(envVars.APPLE_CLIENT_ID, 10, 100, 'APPLE_CLIENT_ID'),
...
validate(
  'RESEND_API_KEY',
  !envVars.RESEND_API_KEY || validateKey(envVars.RESEND_API_KEY, 're_', 'RESEND_API_KEY'),
  'Must start with "re_"',
...
validate(
  ...
  !envVars.NEXT_PUBLIC_GA_MEASUREMENT_ID || validateKey(envVars.NEXT_PUBLIC_GA_MEASUREMENT_ID, 'G-', 'NEXT_PUBLIC_GA_MEASUREMENT_ID'),
  'Must start with "G-"',
...
validate(
  ...
  !envVars.NEXT_PUBLIC_META_PIXEL_ID || /^\d+$/.test(envVars.NEXT_PUBLIC_META_PIXEL_ID),
