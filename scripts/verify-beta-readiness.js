...
filesToCheck.forEach((file) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      content.includes("Estimate only") ||
...
const reportResults = [
  {
    ...
    details: {
      count: report.checks.safetyDisclaimers.count,
      files: report.checks.safetyDisclaimers.files,
    },
  },
  {
    ...
    details: {
      count: report.checks.analyticsIntegration.count,
      files: report.checks.analyticsIntegration.files,
    },
  },
  {
    ...
    details: {
      count: report.checks.edgeCaseTests.count,
      files: report.checks.edgeCaseTests.files,
    },
  },
  {
    ...
    details: {
      count: report.checks.rlsPolicies.count,
      files: report.checks.rlsPolicies.files,
    },
  },
];
...
const detailedReport = generateDetailedReport(
  ...
  failed,
  {
    ...
    additionalInfo: {
      "Overall Status":
        passed === reportResults.length ? "✅ Ready" : "⚠️ Needs Attention",
    },
  }
);
...
console.log("========================");
console.log(
  `Safety Disclaimers: ${report.checks.safetyDisclaimers.count} files`
);
console.log(
  `Analytics Integration: ${report.checks.analyticsIntegration.count} files`
);
...
const reportFile = path.join(
  reportDir,
  `beta-readiness-verification-${Date.now()}.json`
);
