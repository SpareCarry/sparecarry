...
function validateIOS(ipaPath) {
  ...
  if (!fs.existsSync(ipaPath)) {
    console.error(
      `${colors.red}❌ IPA file not found: ${ipaPath}${colors.reset}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      execSync(
        `powershell -Command "Expand-Archive -Path '${ipaPath}' -DestinationPath '${extractDir}' -Force"`,
        { stdio: "ignore" }
      );
    }
    ...
    if (!fs.existsSync(payloadDir)) {
      validate(
        "IPA structure",
        false,
        "Payload directory not found in IPA",
        null
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        const bundleIdMatch = plistText.match(
          /<key>CFBundleIdentifier<\/key>\s*<string>([^<]+)<\/string>/
        );
        const versionMatch = plistText.match(
          /<key>CFBundleShortVersionString<\/key>\s*<string>([^<]+)<\/string>/
        );
        const buildMatch = plistText.match(
          /<key>CFBundleVersion<\/key>\s*<string>([^<]+)<\/string>/
        );

        if (bundleIdMatch) plistData.CFBundleIdentifier = bundleIdMatch[1];
        if (versionMatch)
          plistData.CFBundleShortVersionString = versionMatch[1];
        if (buildMatch) plistData.CFBundleVersion = buildMatch[1];
      } catch {
        // Binary plist, need plutil or other tool
        console.log(
          `${colors.yellow}⚠️  Cannot parse binary plist without plutil${colors.reset}`
        );
      }
    }
    ...
    const bundleId =
      plistData.CFBundleIdentifier || plistData["CFBundleIdentifier"];
    if (bundleId) {
    ...
    validate(
      ...
      !!version,
      "CFBundleShortVersionString not found",
      null
    );
    ...
    const mobileProvisionPath = path.join(
      appBundle,
    ...
    if (fs.existsSync(mobileProvisionPath)) {
      ...
      try {
        ...
        const teamIdMatch = provisionText.match(
          /TeamIdentifier.*?<array>.*?<string>([^<]+)<\/string>/s
        );
        const profileNameMatch = provisionText.match(
          /Name.*?<string>([^<]+)<\/string>/
        );
        ...
        if (profileTypeMatch) {
          validate(
            ...
            true,
            null,
            "Development/Adhoc profile detected"
          );
        } else {
          validate(
            "Provisioning profile type",
            true,
            null,
            "App Store/Enterprise profile"
          );
        }
      } catch {
        validate(
          "Provisioning profile readable",
          false,
          "Cannot read provisioning profile",
          null
        );
      }
    } else {
      ...
      if (fs.existsSync(configFile)) {
        try {
          ...
          if (
            ...
            config.NEXT_PUBLIC_APP_ENV === "staging"
          ) {
            envVarsFound = true;
            validate("Environment variables embedded", true, null, null);
            console.log(
              `   APP_ENV: ${config.APP_ENV || config.NEXT_PUBLIC_APP_ENV}`
            );
          }
        } catch {
    ...
    if (!envVarsFound) {
      validate(
        ...
        false,
        "Staging environment variables not found in app bundle",
        null
      );
    }
    ...
    // Check code signing
    try {
      const codesignOutput = execSync(`codesign -dv "${appBundle}" 2>&1`, {
        encoding: "utf-8",
      });
      ...
      validate(
        "Code signing",
        false,
        "Cannot verify code signing (codesign not available)",
        null
      );
    }
    ...
    console.error(
      `${colors.red}Error verifying IPA: ${error.message}${colors.reset}`
    );
    process.exit(1);
  }
}
...
function validateAndroid(aabPath) {
  ...
  if (!fs.existsSync(aabPath)) {
    console.error(
      `${colors.red}❌ AAB file not found: ${aabPath}${colors.reset}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      // Try with PowerShell on Windows
      execSync(
        `powershell -Command "Expand-Archive -Path '${aabPath}' -DestinationPath '${extractDir}' -Force"`,
        { stdio: "ignore" }
      );
    }
    ...
    const altManifestPath = path.join(
      extractDir,
      "manifest",
      "AndroidManifest.xml"
    );
    ...
    validate(
      ...
      fs.existsSync(manifestPathFound),
    ...
    try {
      ...
      const aaptOutput = execSync(`aapt2 dump badging "${aabPath}"`, {
        encoding: "utf-8",
      });
      ...
      try {
        ...
        const versionCodeMatch = manifestText.match(
          /android:versionCode="([^"]+)"/
        );
        const versionNameMatch = manifestText.match(
          /android:versionName="([^"]+)"/
        );
        ...
        validate(
          ...
          false,
          "Cannot parse AndroidManifest.xml (binary format, need aapt2)",
          null
        );
      }
    }
    ...
    if (jadxAvailable && fs.existsSync(dexPath)) {
      console.log(
        `${colors.cyan}Decompiling AAB with jadx to verify BuildConfig...${colors.reset}`
      );

      const jadxOutputDir = path.join(extractDir, "jadx-output");
      try {
        // Decompile with jadx
        execSync(`jadx -d "${jadxOutputDir}" "${aabPath}"`, {
          stdio: "ignore",
        });
        ...
        const altBuildConfigPath = path.join(
          jadxOutputDir,
          "com",
          "carryspace",
          "app",
        ...
        if (fs.existsSync(buildConfigFile)) {
          ...
          const appEnvMatch = buildConfigContent.match(
            /APP_ENV\s*=\s*"([^"]+)"/
          );
          const supabaseUrlMatch = buildConfigContent.match(
            /SUPABASE_URL\s*=\s*"([^"]+)"/
          );
          const stripeKeyMatch = buildConfigContent.match(
            /STRIPE_PUBLISHABLE_KEY\s*=\s*"([^"]+)"/
          );
          const sentryDsnMatch = buildConfigContent.match(
            /SENTRY_DSN\s*=\s*"([^"]+)"/
          );
          const unleashUrlMatch = buildConfigContent.match(
            /UNLEASH_URL\s*=\s*"([^"]+)"/
          );
          ...
          if (appEnvMatch) {
            ...
            validate(
              "BuildConfig APP_ENV",
              false,
              "APP_ENV not found in BuildConfig",
              null
            );
          }
          ...
          if (supabaseUrlMatch) {
            ...
            validate(
              "BuildConfig SUPABASE_URL",
              false,
              "SUPABASE_URL not found in BuildConfig",
              null
            );
          }
          ...
          if (stripeKeyMatch) {
            ...
            console.log(
              `   STRIPE_PUBLISHABLE_KEY: ${stripeKey.substring(0, 20)}...`
            );
          } else {
            validate(
              "BuildConfig STRIPE_PUBLISHABLE_KEY",
              false,
              "STRIPE_PUBLISHABLE_KEY not found in BuildConfig",
              null
            );
          }
          ...
          if (sentryDsnMatch) {
            ...
            validate(
              "BuildConfig SENTRY_DSN",
              false,
              "SENTRY_DSN not found in BuildConfig",
              null
            );
          }
          ...
          if (unleashUrlMatch) {
            ...
            validate(
              ...
              unleashUrl.length > 0 &&
                (unleashUrl.startsWith("http://") ||
                  unleashUrl.startsWith("https://")),
            ...
            validate(
              "BuildConfig UNLEASH_URL",
              false,
              "UNLEASH_URL not found in BuildConfig",
              null
            );
          }
          ...
          validate(
            "BuildConfig.java found",
            false,
            "BuildConfig.java not found after decompilation",
            null
          );
        }
      } catch (error) {
        validate(
          "jadx decompilation",
          false,
          `Failed to decompile with jadx: ${error.message}`,
          null
        );
      }
    } else {
      ...
      if (fs.existsSync(resourcesPath) || fs.existsSync(dexPath)) {
        if (!jadxAvailable) {
          validate(
            ...
            true,
            null,
            "Environment variables should be in BuildConfig (jadx not installed, cannot verify)"
          );
          console.log(
            `   ${colors.yellow}⚠️  Install jadx to verify BuildConfig values: brew install jadx (macOS) or download from https://github.com/skylot/jadx${colors.reset}`
          );
        } else {
          validate(
            "Build artifacts exist",
            true,
            null,
            "Environment variables should be in BuildConfig"
          );
        }
      } else {
    ...
    if (fs.existsSync(metaInfDir)) {
      ...
      const certFiles = fs
        .readdirSync(metaInfDir)
        .filter(
          (f) => f.endsWith(".RSA") || f.endsWith(".DSA") || f.endsWith(".EC")
        );
      if (certFiles.length > 0) {
        ...
        try {
          ...
          const keytoolOutput = execSync(
            `keytool -printcert -file "${certPath}"`,
            { encoding: "utf-8" }
          );
        ...
        validate(
          ...
          false,
          "No signing certificates found in META-INF",
          null
        );
      }
    } else {
      validate(
        "META-INF",
        false,
        "META-INF directory not found (AAB not signed)",
        null
      );
    }
    ...
    console.error(
      `${colors.red}Error verifying AAB: ${error.message}${colors.reset}`
    );
    process.exit(1);
  }
}
...
if (!platform || !buildPath) {
  console.error(
    `${colors.red}Usage: node scripts/verify-mobile-build.js [ios|android] <path-to-build-file>${colors.reset}`
  );
  console.error(
    `  Example: node scripts/verify-mobile-build.js ios path/to/app.ipa`
  );
  console.error(
    `  Example: node scripts/verify-mobile-build.js android path/to/app.aab`
  );
  process.exit(1);
}
...
if (platform === "ios") {
  ...
  console.error(
    `${colors.red}Invalid platform: ${platform}. Use 'ios' or 'android'${colors.reset}`
  );
  process.exit(1);
}
