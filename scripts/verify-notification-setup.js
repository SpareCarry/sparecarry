...
const NOTIFICATIONS_EMAIL_FROM = process.env.NOTIFICATIONS_EMAIL_FROM || 'SpareCarry <notifications@sparecarry.com>';
...
if (EXPO_ACCESS_TOKEN) {
  if (EXPO_ACCESS_TOKEN.length > 20) {
    ...
    console.log(`   ✅ Token preview: ${EXPO_ACCESS_TOKEN.substring(0, 10)}...`);
  } else {
    console.log('   ⚠️  EXPO_ACCESS_TOKEN seems too short (should be ~40+ chars)');
    allGood = false;
  }
} else {
...
if (RESEND_API_KEY) {
  if (RESEND_API_KEY.startsWith('re_')) {
    ...
    console.log('   ⚠️  RESEND_API_KEY format might be incorrect (should start with re_)');
    ...
    (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200 || res.statusCode === 400) {
          ...
          console.log('   ✅ Expo API connection successful (auth token is valid)');
        } else {
      ...
      (res) => {
        if (res.statusCode === 200 || res.statusCode === 401) {
          ...
          if (res.statusCode === 401) {
            console.log('   ⚠️  Resend API connection works, but auth may be invalid');
          } else {
...
function printSummary() {
  ...
  if (allGood) {
    ...
    console.log('   1. Make sure your Supabase database has the required columns');
    console.log('   2. Test with: node scripts/test-notifications.js --type=both --recipientId=user-id');
    console.log('   3. Check that users have expo_push_token in profiles (for push)');
