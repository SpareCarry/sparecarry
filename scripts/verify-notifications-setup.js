...
if (NOTIFICATIONS_EMAIL_FROM) {
  ...
  if (emailMatch) {
    ...
    console.log(`   ⚠️  NOTIFICATIONS_EMAIL_FROM format might be incorrect: ${NOTIFICATIONS_EMAIL_FROM}`);
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  ...
  console.log('   ⚠️  Skipping schema verification (set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY)');
  ...
  supabase
    ...
    .then(({ error }) => {
      if (error) {
        if (error.message.includes('expo_push_token') || error.message.includes('push_notifications_enabled')) {
          console.log('   ❌ profiles table missing expo_push_token or push_notifications_enabled columns');
          results.supabaseSchema.issues.push('profiles table missing required columns');
          results.supabaseSchema.status = 'fail';
          allGood = false;
        } else {
          console.log('   ⚠️  Could not verify profiles table (might be empty)');
        }
      } else {
        console.log('   ✅ profiles table has expo_push_token and push_notifications_enabled columns');
      }
    });
    ...
    .then(({ error }) => {
      if (error) {
        if (error.message.includes('email')) {
          ...
          results.supabaseSchema.issues.push('users table missing email column');
          results.supabaseSchema.status = 'fail';
          allGood = false;
        } else {
      ...
      results.supabaseSchema.status = results.supabaseSchema.issues.length === 0 ? 'pass' : 'fail';
      console.log('');
      checkSupabaseData();
    });
}
...
function checkSupabaseData() {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.log('   ⚠️  Skipping data verification (Supabase credentials not found)');
  ...
  Promise.all([
    ...
    if (usersWithEmail > 0) {
      console.log(`   ✅ Found ${usersWithEmail} user(s) with email addresses`);
    } else {
      console.log('   ⚠️  No users with email addresses found (email notifications won\'t work)');
      results.supabaseData.issues.push('No users with email addresses');
    }

    if (profilesWithTokens > 0) {
      console.log(`   ✅ Found ${profilesWithTokens} profile(s) with push tokens enabled`);
    } else {
      console.log('   ⚠️  No profiles with push tokens found (push notifications won\'t work)');
      results.supabaseData.issues.push('No profiles with push tokens');
    }

    results.supabaseData.status = results.supabaseData.issues.length === 0 ? 'pass' : 'warning';
    console.log('');
    printSummary();
  }).catch((error) => {
...
function printSummary() {
  ...
  console.log(`   ${results.envVars.status === 'pass' ? '✅' : results.envVars.status === 'warning' ? '⚠️' : '❌'} Status: ${results.envVars.status}`);
  ...
  console.log(`   ${results.supabaseSchema.status === 'pass' ? '✅' : results.supabaseSchema.status === 'skip' ? '⏭️' : '❌'} Status: ${results.supabaseSchema.status}`);
  if (results.supabaseSchema.issues.length > 0) {
    results.supabaseSchema.issues.forEach(issue => console.log(`      - ${issue}`));
  }
  console.log('');

  console.log('Supabase Data:');
  console.log(`   ${results.supabaseData.status === 'pass' ? '✅' : results.supabaseData.status === 'warning' ? '⚠️' : results.supabaseData.status === 'skip' ? '⏭️' : '❌'} Status: ${results.supabaseData.status}`);
  if (results.supabaseData.issues.length > 0) {
    results.supabaseData.issues.forEach(issue => console.log(`      - ${issue}`));
  }
  console.log('');

  // Overall status
  const allPass = results.envVars.status === 'pass' && 
                  results.supabaseSchema.status !== 'fail' && 
  ...
  if (allPass) {
    ...
    console.log('   1. Test with: node scripts/test-notifications.js --type=both --recipientId=user-id');
    console.log('   2. Import Postman collection: NOTIFICATION_TEST_COLLECTION.postman_collection.json');
    console.log('   3. Test in production: Send a test notification to a real user\n');
  } else {
...
setTimeout(() => {
  if (results.supabaseSchema.status === 'checking') {
    console.log('   ⚠️  Supabase check timed out (this is OK if credentials are missing)\n');
    results.supabaseSchema.status = 'skip';
    printSummary();
  }
}, 3000);
