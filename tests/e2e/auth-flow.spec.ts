...
test.describe("Authentication Flow", () => {
  ...
  test("should navigate to login from landing page buttons", async ({
    page,
  }) => {
    ...
    const planeButton = page.getByRole("button", {
      name: /I'm traveling by Plane/i,
    });
    ...
    const boatButton = page.getByRole("button", {
      name: /I'm sailing by Boat/i,
    });
      ...
      try {
        ...
        if (
          ...
          if (!/\/auth\/login/.test(finalUrl)) {
            throw new Error(
              `Expected to navigate to /auth/login but URL is ${finalUrl}`
            );
          }
        }
      }
  ...
  test("should request magic link with correct email", async ({ page }) => {
    ...
    await expect(page.getByText("Welcome to CarrySpace"))
    ...
    if (response) {
      console.log(
        ...
        response.status(),
        response.status() === 200 ? "(MOCKED)" : "(REAL)"
      );
      // If we got a non-200 response, the mock didn't work - that's OK, we'll handle it
      if (response.status() !== 200) {
        console.log(
          "[TEST] Note: Real Supabase returned",
          response.status(),
          "- this is expected if mock pattern didn't match"
        );
      }
    } else {
      console.log(
        "[TEST] WARNING: No response received - request may not have been sent"
      );
    }
    ...
    const hasSuccessDiv =
      pageContent.includes("bg-teal-50") ||
    ...
    try {
      await expect(
        successMessage.or(successDiv.first()).or(anyMessage.first())
      ).toBeVisible({ timeout: 10000 });
    } catch (e) {
      ...
      const messageVisible = await anyMessageDiv
        .first()
        .isVisible()
        .catch(() => false);
      if (!messageVisible) {
        ...
        await page
          .screenshot({
            path: "test-results/debug-success-message.png",
            fullPage: true,
          })
          .catch(() => {});
        throw new Error(
          'Success message not found. Expected "Check your email" message after OTP request.'
        );
      }
  ...
  test("should handle magic link callback with code", async ({
    page,
    context,
  }) => {
    ...
    await page
      .waitForURL(/\/(home|auth\/login|auth\/callback)/, { timeout: 10000 })
      .catch(() => {
    ...
    try {
      await page.waitForURL(/\/(home|auth\/login|auth\/callback)/, {
        timeout: 5000,
      });
    } catch (e) {
  ...
  test("should handle authentication errors gracefully", async ({ page }) => {
    ...
    const errorMessage = page.locator(
      "text=/authentication failed|test error/i"
    );
  ...
  test("should preserve redirect parameter through auth flow", async ({
    ...
    await page.goto(
      `${baseUrl}/auth/login?redirect=${encodeURIComponent(redirectPath)}`
    );
