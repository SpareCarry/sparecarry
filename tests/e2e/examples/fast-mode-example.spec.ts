...
test.describe('Fast Mode Examples', () => {
  ...
  test('should load home page with authenticated user', async ({ page }) => {
    ...
    await expect(page.locator('text=Sign Out').first()).toBeVisible({ timeout: 15000 });
    
    // Should see home content
    await expect(page.locator('text=SpareCarry').first()).toBeVisible();
  });

  test('should display user profile with subscription options', async ({ page }) => {
    await enableTestMode(page, USER_A);
  ...
  test('should show subscription options for monthly user', async ({ page }) => {
    const monthlyUser = {
      ...USER_A,
      userData: {
        ...USER_A.userData,
  ...
  test('should switch between users mid-test', async ({ page }) => {
    ...
    await page.goto(`${baseUrl}/home/profile`, { waitUntil: 'domcontentloaded' });
    await expect(page.locator(`text=${USER_B.email}`).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test('should test subscription purchase flow', async ({ page }) => {
    ...
    const subscribeButton = page.locator('button:has-text("Subscribe Monthly")').first();
  ...
  test('should test job posting flow', async ({ page }) => {
    ...
    const hasFormElements = await page.locator('input, select, textarea, button[type="submit"]').count();
