...
test.describe('Beta Testing Flow', () => {
  ...
  test('should complete full user journey: signup → post request → messaging', async ({ page }) => {
    ...
    await expect(page.locator('#title').first()).toHaveValue('Test Item for Beta');
    await expect(categorySelect).toContainText('Electronics');
  });

  test('should complete shipping estimator → job creation flow', async ({ page }) => {
    await enableTestMode(page, USER_A);
  ...
  test('should handle multi-user messaging scenario', async ({ page }) => {
    ...
    if (await postItem.isVisible().catch(() => false)) {
      ...
      const hasOpenMessages = await openMessagesButton.isVisible().catch(() => false);
  ...
  test('should verify all safety disclaimers are present', async ({ page }) => {
    ...
    const safetyText = page.getByText(/All communication stays on SpareCarry/i).first();
