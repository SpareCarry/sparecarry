...
const selectCategoryOption = async (
  page: Page,
...
const getEmergencyText = async (page: Page) => {
  const paragraph = page
    .locator("p")
    .filter({ hasText: /Emergency Mode Active/i })
    .first();
...
test.describe("Edge Cases & Safety Checks", () => {
  ...
  test("should enforce boat-only transport for restricted items", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Restricted items can only be transported by boat/i)
    ).toBeVisible({ timeout: 5000 });
  });

  test("should apply correct emergency multiplier with cap", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
    ...
    if (!emergencyTextCapped || !emergencyTextCapped.includes("Emergency")) {
      ...
      const emergencyIndicator = page
        .locator("text=/Emergency/i")
        .or(page.locator('[class*="emergency"]'))
        .first();
      const hasIndicator = await emergencyIndicator
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      expect(
        hasIndicator ||
          (emergencyTextCapped && emergencyTextCapped.includes("Emergency"))
      ).toBe(true);
    } else {
  ...
  test("should validate location selection methods", async ({ page }) => {
    ...
    const locationInput = page
      .locator('input[placeholder*="Departing"]')
      .first();
  ...
  test("should calculate shipping estimator correctly with all inputs", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
    ...
    await selectCountry(page, "origin_country", "United States").catch(
      async () => {
        await page.waitForTimeout(500);
        await selectCountry(page, "origin_country", "United States");
      }
    );
    await selectCountry(page, "destination_country", "Australia").catch(
      async () => {
        await page.waitForTimeout(500);
    ...
    await expect(page.getByText("Courier (DHL)", { exact: true })).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should display safety disclaimers correctly", async ({ page }) => {
    ...
    const hasCustomsDisclaimer = await customsDisclaimer
      .isVisible()
      .catch(() => false);
    ...
    const restrictedDisclaimer = page
      .getByText(/Restricted items cannot be transported by plane/i)
      .first();
    const hasRestrictedDisclaimer = await restrictedDisclaimer
      .isVisible()
      .catch(() => false);
    // Disclaimer appears when restricted items checkbox is checked

    // Check for emergency disclaimer
    const emergencyDisclaimer = page
      .getByText(/This extra reward helps prioritize/i)
      .first();
    const hasEmergencyDisclaimer = await emergencyDisclaimer
      .isVisible()
      .catch(() => false);
    // Disclaimer appears when emergency checkbox is checked
  });

  test("should validate category selection and other option", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
    ...
    const otherDescription = page
      .locator("#category_other_description")
      .first();
