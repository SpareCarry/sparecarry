...
test.describe("Idea Suggestion Flow", () => {
  ...
  test("user can navigate to suggest idea from profile", async ({ page }) => {
    ...
    await expect(page.locator('text=Suggest an Idea').or(page.getByText(/Suggest an Idea/i)).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("user can submit an idea suggestion", async ({ page }) => {
    ...
    // Submit the form
    const submitButton = page.locator('button:has-text("Submit Idea")').or(page.getByRole('button', { name: /Submit Idea/i }));
    ...
    await page.waitForFunction(
      () => {
        ...
        return text.includes('Idea Submitted!') || text.includes('Idea Submitted');
      },
      { timeout: 20000 }
    ).catch(() => {});
    
    // Check for success message with flexible selector
    const successMessage = page.locator('text=Idea Submitted!')
