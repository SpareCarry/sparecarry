...
test.describe("Job Posting Flow", () => {
  ...
  test("should show form fields for posting trip", async ({ page }) => {
    ...
    const hasFormElements = await page
      .locator(
        'input, select, textarea, button[type="submit"], button, [role="textbox"], [role="combobox"], [contenteditable="true"]'
      )
      .count();
  ...
  test("should allow filling trip details", async ({ page }) => {
    ...
    const hasFlightNumber = await flightNumberRadio
      .isVisible()
      .catch(() => false);
    ...
    const hasDepartureLocation = await departureLocation
      .isVisible()
      .catch(() => false);
    ...
    await page
      .waitForSelector(
        'label:has-text("Arrival Location"), *:has-text("Arrival Location")',
        {
          timeout: 20000,
          state: "visible",
        }
    ...
    const hasArrivalLocation = await arrivalLocation
      .isVisible()
      .catch(() => false);
    ...
    const prohibitedCheckbox = page.getByText(
      /I confirm I am not transporting prohibited items/i
    );
    const hasProhibitedCheckbox = await prohibitedCheckbox
      .isVisible()
      .catch(() => false);
  ...
  test("should require prohibited items confirmation", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (formElement) {
        ...
        const departureInputs = Array.from(
          document.querySelectorAll(
            'input[placeholder*="Departure" i], input[id*="from" i]'
          )
        );
        const arrivalInputs = Array.from(
          document.querySelectorAll(
            'input[placeholder*="Arrival" i], input[id*="to" i]'
          )
        );
        ...
        if (departureInputs.length > 0) {
          ...
          departureInputs[0].dispatchEvent(
    ...
    const spareKgInput = page
      .locator('input[id*="spare_kg" i], input[name*="spare_kg" i]')
      .first();
    ...
    const lengthInput = page
      .locator('input[id*="length" i], input[name*="length" i]')
      .first();
    ...
    const widthInput = page
      .locator('input[id*="width" i], input[name*="width" i]')
      .first();
    ...
    const heightInput = page
      .locator('input[id*="height" i], input[name*="height" i]')
      .first();
    ...
    const prohibitedCheckboxInput = page
      .locator("#prohibited_items_confirmed")
      .first();
    const isChecked = await prohibitedCheckboxInput
      .isChecked()
      .catch(() => false);
    if (isChecked) {
    ...
    expect(
      isChecked ||
        !(await prohibitedCheckboxInput.isChecked().catch(() => false))
    ).toBe(true);

    // Try to submit without checking prohibited items checkbox

    const submitButton = page
      .getByRole("button", { name: /Post Trip/i })
      .first();
    if (await submitButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const prohibitedItemsError = page
        .getByText(
          /You must confirm that you are not carrying prohibited items/i
        )
      ...
      const hasValidationError = await page.evaluate(() => {
        ...
        const errorElements = Array.from(
          document.querySelectorAll("p.text-red-600, .text-red-600")
        );
        return errorElements.some((el) => {
          const text = el.textContent?.toLowerCase() || "";
          return (
            text.includes("prohibited") ||
            (text.includes("confirm") && text.includes("carrying"))
          );
        });
      });

      // Try to find the error in the DOM
      const errorVisible = await prohibitedItemsError
        .first()
      ...
      if (!errorVisible && !hasValidationError) {
        ...
        if (checkboxExists) {
          ...
          throw new Error(
            "Prohibited items checkbox not found and error message not visible."
          );
        }
      } else {
        // Error is visible or exists in validation state - success!
        if (errorVisible) {
          await expect(prohibitedItemsError.first()).toBeVisible({
            timeout: 5000,
          });
        }
