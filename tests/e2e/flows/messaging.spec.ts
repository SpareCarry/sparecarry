...
test.describe('Messaging System', () => {
  ...
  test('should show chat button for involved users on post detail', async ({ page }) => {
    ...
    if (await postItem.isVisible().catch(() => false)) {
      ...
      expect(hasChatButton || page.getByText(/Message/i).isVisible()).toBeTruthy();
    }
  });

  test('should open message thread modal when chat button clicked', async ({ page }) => {
    await enableTestMode(page, USER_A);
  ...
  test('should display message badge in sidebar when unread messages exist', async ({ page }) => {
    await enableTestMode(page, USER_A);
    ...
    if (viewport && viewport.width >= 1024) {
      ...
      const messageBadge = page.locator('[aria-label*="unread message"]').first();
    ...
    if (await postItem.isVisible().catch(() => false)) {
      ...
      if (await chatButton.isVisible().catch(() => false)) {
        ...
        if (await sendButton.isVisible().catch(() => false)) {
          ...
          await expect(page.getByText(/Test message from E2E/i)).toBeVisible({ timeout: 5000 });
        }
      }
    }
  });
  ...
  test('should show safety tooltip in message input', async ({ page }) => {
    ...
    if (await postItem.isVisible().catch(() => false)) {
      ...
      if (await chatButton.isVisible().catch(() => false)) {
        ...
        await expect(page.getByText(/All communication stays on SpareCarry/i)).toBeVisible({ timeout: 5000 });
      }
    }
  });
});
