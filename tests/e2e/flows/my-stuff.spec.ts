...
test.describe("My Stuff Page", () => {
  ...
  test("should navigate to my stuff page", async ({ page }) => {
    ...
    const heading = page
      .locator('h1:has-text("My Stuff")')
      .or(page.getByRole("heading", { name: "My Stuff", exact: true }));
    await expect(heading.first()).toBeVisible({ timeout: 15000 });
  });

  test("should show only one support button with correct email link", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
  ...
  test("should have accessible support button", async ({ page }) => {
    ...
    const supportButton = page
      ...
      .or(page.locator('button:has-text("Contact Support")'));
  ...
  test("should open email client when support button is clicked", async ({
    page,
    context,
  }) => {
    await enableTestMode(page, USER_A);
