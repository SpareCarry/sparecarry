...
test.describe("Post Request Upgrades", () => {
  ...
  test("should display category dropdown with options", async ({ page }) => {
    ...
    await expect(
      page.getByRole("option", { name: "Electronics" })
    ).toBeVisible();
    await expect(
      page.getByRole("option", { name: "Marine Equipment" })
    ).toBeVisible();
    await expect(page.getByRole("option", { name: /^Other$/ })).toBeVisible();
  });

  test('should show "Other" description field when "Other" is selected', async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
  ...
  test("should show restricted items checkbox and prevent plane transport", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
    ...
    if (!isDisabled) {
      ...
      expect(
        isPlaneDisabled || !(await planeOption.isVisible().catch(() => false))
      ).toBeTruthy();
    }
  });
  ...
  test("should calculate emergency pricing correctly", async ({ page }) => {
    ...
    await expect(page.getByText(/Emergency.*\+25%/i))
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
        // Alternative: check for emergency mode active text
        expect(page.getByText(/Emergency Mode Active/i)).toBeVisible({
          timeout: 5000,
        });
      });
  });
  ...
  test("should cap emergency bonus at $15", async ({ page }) => {
    ...
    const emergencyParagraph = page
      .locator("p")
      .filter({ hasText: /Emergency Mode Active/i })
      .first();
    const emergencyText = await emergencyParagraph
      .textContent()
      .catch(() => "");
  ...
  test("should display updated Buy & Ship Directly wording", async ({
    page,
  }) => {
    await enableTestMode(page, USER_A);
    ...
    await expect(
      page.getByText(/User purchases item from retailer/i)
    ).toBeVisible();
    await expect(
      page.getByText(/SpareCarry delivers to traveler/i)
    ).toBeVisible();
  });
