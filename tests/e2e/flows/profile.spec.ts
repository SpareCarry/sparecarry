...
test.describe('Profile Flow', () => {
  ...
  test('should load profile page', async ({ page }) => {
    ...
    const profileHeading = page.locator('h1:has-text("Profile"), [role="heading"]:has-text("Profile")').first();
  ...
  test('should display user email', async ({ page }) => {
    ...
    const emailPattern = new RegExp(USER_A.email.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    await expect(page.getByText(emailPattern).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test('should display subscription card', async ({ page }) => {
    ...
    const subscriptionCard = page.locator('[data-testid="sparecarry-pro-title"]')
