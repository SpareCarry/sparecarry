...
test.describe("Complete Payment Flow", () => {
  ...
  test.skip(
    process.env.CI === "true",
    "Skipping E2E payment flow in CI - requires manual testing"
  );
      ...
      try {
        ...
        if (
          ...
          if (!/\/auth\/login/.test(finalUrl) && !/\/home/.test(finalUrl)) {
            throw new Error(
              `Expected to navigate to /auth/login or /home but URL is ${finalUrl}`
            );
          }
        }
      }
  ...
  test("browse page should load", async ({ page }) => {
    ...
    await page
      .waitForURL(/\/(home|auth\/login)/, { timeout: 15000 })
      .catch(() => {});
