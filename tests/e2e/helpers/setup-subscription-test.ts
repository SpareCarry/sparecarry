...
export async function setupSubscriptionTest(
  page: Page,
  user: TestUser,
  options?: {
  ...
  await page.route(
    ...
    async (route) => {
      ...
      const isLifetime =
        user.profile?.lifetime_active === true ||
        user.userData?.lifetime_pro === true;
      ...
      const responseBody = [
        {
          ...
          stripe_identity_verified_at:
            user.profile?.stripe_identity_verified_at || null,
      ...
      console.log(
        `[SETUP_SUBSCRIPTION_TEST] ✓ Intercepted ${request.method()} ${request.url()}`
      );
      console.log(
        `[SETUP_SUBSCRIPTION_TEST] Response: isLifetime=${isLifetime}, lifetime_active=${responseBody[0].lifetime_active}`
      );
  ...
  await page.route(
    (url) => {
      const href = typeof url === "string" ? url : url.href;
      return (
        href.includes("/rest/v1/users") &&
        !href.includes("/rest/v1/users_count")
      );
    },
    async (route) => {
      const request = route.request();
      ...
      const responseBody = [
        {
          ...
          lifetime_pro_purchased_at: isLifetime
      ...
      console.log(
        `[SETUP_SUBSCRIPTION_TEST] Response: lifetime_pro=${responseBody[0].lifetime_pro}, subscription_status=${responseBody[0].subscription_status}`
      );
