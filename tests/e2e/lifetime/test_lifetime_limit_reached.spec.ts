...
test.describe("Lifetime Limit Reached", () => {
  ...
  test("should hide lifetime option on pricing page when limit reached", async ({ page }) => {
    ...
    await page.waitForSelector('h1:has-text("SpareCarry Pro"), [data-testid="subscription-card"]', { 
      timeout: 20000 
    }).catch(() => {});
    ...
    const lifetimeCard = page.locator('text=Buy Lifetime').or(page.locator('text=Limited'));
    await expect(lifetimeCard).not.toBeVisible({ timeout: 5000 });

    // Verify monthly and yearly options are still visible
    await expect(page.locator('button:has-text("Subscribe")').first()).toBeVisible({ timeout: 10000 });
  });

  test("should skip lifetime offer screen when limit reached", async ({ page }) => {
    ...
    await page.waitForSelector('h1, [data-testid="onboarding"], text=/Phone Verification|Identity Verification/i', { 
      timeout: 20000 
    }).catch(() => {});
    ...
    const lifetimeOffer = page.locator('text=Support SpareCarry').or(page.locator('text=Get Lifetime Access'));
