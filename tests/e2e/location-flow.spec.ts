...
test.describe('Location System', () => {
  ...
  test('should show location autocomplete in request form', async ({ page }) => {
    ...
    const departureLabel = page.getByText(/Departure Location/i).or(
      page.locator('label:has-text("Departure Location")')
    ).or(
      page.locator('*:has-text("Departure Location")')
    );
    await expect(departureLabel.first()).toBeVisible({ timeout: 20000 });

    const arrivalLabel = page.getByText(/Arrival Location/i).or(
      page.locator('label:has-text("Arrival")')
    ).or(
      page.locator('*:has-text("Arrival Location")')
    );
  ...
  test('should filter marinas when marina toggle enabled', async ({ page }) => {
    ...
    const locationInput = page.locator('input[placeholder*="Departing" i], input[placeholder*="location" i]').first();
  ...
  test('should show current location button', async ({ page }) => {
    ...
    const buttonExists = await currentLocationButton.isVisible().catch(() => false);
