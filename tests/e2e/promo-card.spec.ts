...
test.describe("Promo Card System", () => {
  ...
  test("should render Early Supporter promo card when daysLeft > 0", async ({
    page,
  }) => {
    ...
    await page
      .waitForFunction(
        () => {
          ...
          return (
            text.includes("Early Supporter") || text.includes("0% platform")
          );
        },
        { timeout: 20000 }
      )
  ...
  test("should allow dismissing promo card", async ({ page }) => {
    ...
    const dismissButton = page
      .locator('button[aria-label="Dismiss promo"]')
      .first();
    if (await dismissButton.isVisible()) {
      ...
      await expect(
        page.locator("text=Early Supporter Reward")
      ).not.toBeVisible();
    }
  });
  ...
  test("should show First Delivery promo after promo ends for users with no deliveries", async ({
    page,
  }) => {
