...
import { selectCountry, clickAndWaitForNavigation } from './helpers/test-helpers';
...
test.describe('Shipping Cost Estimator', () => {
  ...
  test('should open estimator screen and calculate prices', async ({ page }) => {
    ...
    const lengthInput = page.locator('input[id="length"], input[placeholder*="length" i]').first();
    ...
    const widthInput = page.locator('input[id="width"], input[placeholder*="width" i]').first();
    ...
    const heightInput = page.locator('input[id="height"], input[placeholder*="height" i]').first();
    ...
    // Fill in weight
    const weightInput = page.locator('input[id="weight"], input[placeholder*="weight" i]').first();
    ...
    if (hasComparison) {
      ...
      await expect(page.getByText(/SpareCarry/i).first()).toBeVisible({ timeout: 5000 });
      
      // Verify "No site fees" text is NOT displayed (removed from standard estimates)
      const noSiteFees = page.getByText(/No site fees/i);
      const hasNoSiteFees = await noSiteFees.first().isVisible().catch(() => false);
      expect(hasNoSiteFees).toBe(false);
      
      // Verify savings percentage is > 20%
      const savingsText = await page.getByText(/Save \d+%/i).first().textContent().catch(() => '');
      if (savingsText) {
  ...
  test('should navigate to job creation with prefill data', async ({ page }) => {
    await enableTestMode(page, USER_A);
    ...
    if (buttonExists) {
        ...
        await expect(sidebarLink).toBeVisible();
          ...
          if (prefillData.stripeFeePlane !== undefined) {
            ...
            if (prefillData.platformFeePlane && prefillData.stripeFeePlane) {
              const expectedNetRevenue = prefillData.platformFeePlane - prefillData.stripeFeePlane;
              expect(prefillData.netRevenuePlane).toBeCloseTo(expectedNetRevenue, 2);
            }
          }
        } catch (e) {
      ...
      console.log('Create Job button not visible - form may need to be filled first');
    }
  });
  ...
  test('should show Premium Price card for non-premium users', async ({ page }) => {
    await enableTestMode(page, USER_A);
  ...
  test('should validate premium discount calculation (flat fee waived + % fee halved)', async ({ page }) => {
    await enableTestMode(page, USER_A);
  ...
  test('should calculate and display customs cost when declared value is entered', async ({ page }) => {
    await enableTestMode(page, USER_A);
    ...
    const declaredValueInput = page.locator('input[id="declared_value"]').first();
  ...
  test('should prefill max_reward slider when creating job from estimate', async ({ page }) => {
    await enableTestMode(page, USER_A);
    ...
    const createJobButton = page.getByRole('button', { name: /Create SpareCarry Job from This Estimate/i });
    ...
    if (await maxRewardSlider.isVisible({ timeout: 10000 }).catch(() => false)) {
