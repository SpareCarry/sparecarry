...
test.describe("Trusted Traveller Badge", () => {
  ...
  test("should display trusted traveller badge on profile when user has badge", async ({
    page,
  }) => {
    ...
    await page.route(
      (url) => url.href.includes("/rest/v1/traveller_stats"),
      async (route) => {
        await route.fulfill({
          status: 200,
    ...
    await page.route(
      (url) => url.href.includes("/rest/v1/user_badges"),
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
