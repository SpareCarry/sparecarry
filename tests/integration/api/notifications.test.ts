...
describe("Notification Endpoints", () => {
  ...
  const BASE_URL = process.env.NEXT_PUBLIC_APP_URL?.startsWith(
    "http://localhost"
  )
  ...
  it(
    "should have register-token endpoint",
    async () => {
      try {
  ...
  it(
    "should have send-message endpoint",
    async () => {
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 3000);
        ...
        const response = await fetch(
          `${BASE_URL}/api/notifications/send-message`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  it(
    "should have send-match endpoint",
    async () => {
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 3000);
        ...
        const response = await fetch(
          `${BASE_URL}/api/notifications/send-match`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
