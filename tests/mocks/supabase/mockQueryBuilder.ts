...
export function createMockQueryBuilder(table: string): MockQueryBuilder {
        ...
        case "gt":
          if (filter.column) {
            result = result.filter((item: unknown) => {
              const record = item as Record<string, unknown>;
              return (
                (record[filter.column!] as number) > (filter.value as number)
              );
            });
          }
          break;
  ...
  const builder: MockQueryBuilder = {
    ...
    then: async <T>(
      callback: (result: { data: T[] | null; error: null }) => T
    ) => {
      ...
      if (query.type === "upsert" && query.data) {
        ...
        const upserted = dataArray.map((item) => {
          ...
          if (record.id) {
            ...
            if (existing) {
              return updateMockData(table, record.id as string, {
                ...record,
                updated_at: new Date().toISOString(),
              });
            }
